...
DEFAULTS = {
    ...
    "scale_hi": 3.0,             # 上限
    # R33 §3：刻みはセル幅と参照の広がりから導く。独立に決められない。
    #   縮尺が δ ずれると距離 L の対向面の間隔が δL ずれる。これがセル幅を
    #   超えると整数セルの平行移動では両面を同時に合わせられない。
    #   δ ≤ c/(2L) → N = ceil(ln(s_max/s_min)/ln(1+2δ)) + 1。
    #   c=0.25 m・L=18.2 m（実データの参照の広がり）で N=170。
    #   **R29 §2-1 の 59 点はこの関係を満たしていなかった（刻み 4.05%）。**
    "scale_n": 170,              # 対数間隔の点数（R33 §3-3 で 59 から変更）
