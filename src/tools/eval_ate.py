...
#
# This file is a modified version of https://github.com/idiap/ESLAM/blob/main/src/tools/eval_ate.py
...
def evaluate_ate(first_list, second_list, plot="", _args=""):
    ...
    return {
        ...
        "absolute_translational_error.rmse": (numpy.sqrt(numpy.dot(trans_error, trans_error) / len(trans_error))*100),
        "absolute_translational_error.mean": (numpy.mean(trans_error)*100),
        "absolute_translational_error.median": (numpy.median(trans_error)*100),
        "absolute_translational_error.std": (numpy.std(trans_error)*100),
        "absolute_translational_error.min": (numpy.min(trans_error)*100),
        "absolute_translational_error.max": (numpy.max(trans_error)*100),
    }
