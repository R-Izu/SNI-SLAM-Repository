...
class Frame_Visualizer(object):
    ...
    def save_imgs(self, idx, iter, gt_depth, gt_color, c2w_or_camera_tensor, all_planes, decoders, gt_sem=None, est_sem=None):
        ...
        with torch.no_grad():
            if (idx % self.freq == 0) and (iter % self.inside_freq == 0):
                ...
                # セマンティック情報がある場合: 行0=semantic, 行1=color, 行2=depth
                # セマンティック情報がない場合: 行0=depth, 行1=color
                depth_row = 2 if gt_sem is not None else 0
                color_row = 1
                ...
                axs[depth_row, 0].imshow(gt_depth_np, cmap="plasma", vmin=0, vmax=max_depth)
                axs[depth_row, 0].set_xticks([])
                axs[depth_row, 0].set_yticks([])
                axs[depth_row, 1].imshow(depth_np, cmap="plasma", vmin=0, vmax=max_depth)
                axs[depth_row, 1].set_xticks([])
                axs[depth_row, 1].set_yticks([])
                axs[depth_row, 2].imshow(depth_residual, cmap="plasma", vmin=0, vmax=max_depth)
                axs[depth_row, 2].set_xticks([])
                axs[depth_row, 2].set_yticks([])
                gt_color_np = np.clip(gt_color_np, 0, 1)
                color_np = np.clip(color_np, 0, 1)
                color_residual = np.clip(color_residual, 0, 1)
                axs[color_row, 0].imshow(gt_color_np, cmap="plasma")
                axs[color_row, 0].set_xticks([])
                axs[color_row, 0].set_yticks([])
                axs[color_row, 1].imshow(color_np, cmap="plasma")
                axs[color_row, 1].set_xticks([])
                axs[color_row, 1].set_yticks([])
                axs[color_row, 2].imshow(color_residual, cmap="plasma")
                axs[color_row, 2].set_xticks([])
                axs[color_row, 2].set_yticks([])
    ...
    def decode_segmap(self, image, nc=6):
        label_colors = np.array([
            (128, 128, 128),  # 0: background (gray)
            (255,  64,  64),  # 1: wall        (red)
            (255, 200,  64),  # 2: door        (amber)
            (180, 220, 255),  # 3: floor       (light blue)
            ( 64, 200, 255),  # 4: window      (cyan)
            (200, 100, 255),  # 5: ceiling     (purple)
        ], dtype=np.uint8)
